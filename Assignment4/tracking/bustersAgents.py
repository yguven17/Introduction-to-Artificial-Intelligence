...
class GreedyBustersAgent(BustersAgent):
    ...
    def chooseAction(self, gameState):
        ...
        minAction = 0
        for action in legal:
            if minValue > min(self.distancer.getDistance(Actions.getSuccessor(pacmanPosition, action), ghost) for ghost in
                      maxBelief):
                minAction = action
                minminValueval = min(self.distancer.getDistance(Actions.getSuccessor(pacmanPosition, action), ghost) for ghost in
                      maxBelief)
        return minAction