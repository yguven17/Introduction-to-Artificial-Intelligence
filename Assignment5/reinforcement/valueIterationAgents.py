    ...
    def runValueIteration(self):
        ...
        for state in self.mdp.getStates():
            if not self.mdp.isTerminal(state):
                ...
                diff = abs(max([self.getQValue(state,action) for action in actions]) - self.getValue(state))
        ...
        for i in range(self.iterations):
            ...
            if not self.mdp.isTerminal(state):
                self.values[state] = max([self.getQValue(state,self.mdp.getPossibleActions(state)) for action in self.mdp.getPossibleActions(state)])

            for predecessor in predecessors[state]:
                if not self.mdp.isTerminal(predecessor):
                    actions = self.mdp.getPossibleActions(predecessor)

                    diff = abs(max([self.getQValue(predecessor,action) for action in actions])(predecessor, actions) - self.getValue(predecessor))

                    if diff > self.theta:
                        p_queue.update(predecessor, -diff)
